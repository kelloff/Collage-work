...
func init(db_manager: Node) -> void:
	...
	if dbm and dbm._ensure_db():
		...
		if not progress_cols.has("status"):
			print("DbTasks.init: progress.status column missing. cols=", progress_cols)

		# Важно для UX:
		# загружаем задания из task_data.gd только на "пустой" БД.
		# При последующих запусках оставляем уже закреплённые задания и прогресс как есть.
		dbm.db.query("SELECT COUNT(*) AS cnt FROM tasks")
		var task_count := 0
		if dbm.db.query_result.size() > 0:
			task_count = int(dbm.db.query_result[0].get("cnt", dbm.db.query_result[0].get("COUNT(*)", 0)))
		if task_count == 0:
			print("DbTasks.init: tasks table is empty -> loading defaults once")
			load_default_tasks_from_file("res://db/task_data.gd")
		else:
			print("DbTasks.init: tasks already exist -> keep existing tasks/progress. count=", task_count)
...
func load_default_tasks_from_file(path: String = "res://db/task_data.gd") -> void:
	...
	# определяем источник только для отладочного лога.
...
## Вставка задач из JSON бэкенда /generate_tasks (после new_game_database_wipe).
## Описания с префиксом AI: — для фильтра в assign_task.
func insert_tasks_from_backend_payload(items: Array) -> int:
	if not dbm or not dbm._ensure_db():
		return 0
	var db = dbm.db
	db.query("PRAGMA table_info(tasks)")
	var task_cols: Array = []
	...
	for t in items:
		if typeof(t) != TYPE_DICTIONARY:
			continue
		var raw_desc := str(t.get("description", "")).strip_edges()
		if raw_desc == "":
			continue
		var desc := raw_desc
		if not desc.begins_with("AI:"):
			desc = "AI: " + desc

		var desc_esc := desc.replace("'", "''")
		db.query("SELECT id FROM tasks WHERE description = '%s'" % desc_esc)
		if db.query_result.size() > 0:
			continue
		...
		if task_cols.has("expected_code"):
			row["expected_code"] = str(t.get("expected_code", ""))
		if task_cols.has("expected_output"):
			row["expected_output"] = str(t.get("expected_output", ""))
		if task_cols.has("required_patterns"):
			row["required_patterns"] = str(t.get("required_patterns", ""))
	...
	print("DbTasks.insert_tasks_from_backend_payload: inserted=", inserted_count)
...
func assign_task(level: int, computer_id: int) -> Dictionary:
	...
	available.shuffle()
	var task: Dictionary = available[0]
