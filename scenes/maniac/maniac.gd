...
@export var wall_escape_min_stuck_s: float = 0.12
@export var wall_escape_blocked_velocity_ratio: float = 0.35
@export var wall_escape_probe_px: float = 14.0
...
var _wall_escape_side: float = 1.0
...
func _follow_nav_agent(delta: float, spd: float) -> void:
	...
	if _wall_escape_cooldown_left > 0.0:
		_wall_escape_cooldown_left -= delta

	var collision := get_last_slide_collision()
	if collision != null and _wall_escape_cooldown_left <= 0.0:
		var moved_px: float = global_position.distance_to(pos_before)
		var blocked: bool = (
			_stuck_elapsed >= wall_escape_min_stuck_s
			or moved_px <= stuck_move_epsilon_px * 0.5
			or velocity.length() < desired_velocity.length() * wall_escape_blocked_velocity_ratio
		)
		if blocked:
			_try_wall_sidestep(collision, desired_velocity)
...
func _pick_wall_sidestep_dir(normal: Vector2, desired_velocity: Vector2) -> Vector2:
	var tangent_left: Vector2 = Vector2(-normal.y, normal.x).normalized()
	var tangent_right: Vector2 = -tangent_left
	var can_left: bool = not test_move(global_transform, tangent_left * wall_escape_probe_px)
	var can_right: bool = not test_move(global_transform, tangent_right * wall_escape_probe_px)
	...
	if can_left and can_right:
		if desired_velocity.length() <= 0.001:
			return tangent_left if _wall_escape_side > 0.0 else tangent_right
		return tangent_left if tangent_left.dot(desired_velocity) >= tangent_right.dot(desired_velocity) else tangent_right
	return tangent_left * _wall_escape_side

func _try_wall_sidestep(collision: KinematicCollision2D, desired_velocity: Vector2) -> void:
	var normal: Vector2 = collision.get_normal()
	if normal.length() <= 0.001:
		return

	var sidestep: Vector2 = _pick_wall_sidestep_dir(normal, desired_velocity)
	var pos_before: Vector2 = global_position
	velocity = sidestep * wall_escape_speed
	move_and_slide()

	if get_last_slide_collision() != null and global_position.distance_to(pos_before) <= stuck_move_epsilon_px:
		_wall_escape_side *= -1.0
		sidestep = -sidestep
		pos_before = global_position
		velocity = sidestep * wall_escape_speed
		move_and_slide()
		if get_last_slide_collision() != null and global_position.distance_to(pos_before) <= stuck_move_epsilon_px:
			_wall_escape_side *= -1.0
