...
func _process(_delta: float) -> void:
	...
	if player_in_range and Input.is_action_just_pressed("interact"):
		if tutorial_step != "" and TutorialManager.try_interact_step(tutorial_step, Callable(self, "open_chest")):
			return
		open_chest()
...
func open_chest() -> void:
	...
	var force_drop := TutorialManager.is_active() and tutorial_step in ["chest_1", "chest_2"]
	if force_drop or rng.randf() <= drop_chance:
		_spawn_random_drop()
